...
1. **Get current status** - Call `ijoka_status` to find the active feature (status: in_progress)
...
3. **Verify completion** - Before marking complete, check:
   ...
   - Are there pending plan steps? (call `ijoka_get_plan`)
   - If incomplete plan, warn: "Plan has uncompleted steps. Mark complete anyway?"

4. **Complete the feature** - Call `ijoka_complete_feature`:
   ```
